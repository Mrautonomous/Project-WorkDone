...
function Cart() {
  ...
  const clearCartHandler = () => {
    clearCart();
    toast.success("Favourite list is empty! ❤️ ");
  };
      ...
      <div className=" m-auto w-[380px] text-white">
        {cartItems.length > 0 ? (
          cartItems.map((item) => {
            ...
            return (
              <Link key={item.id} to={`/store/detail/${item.id}`}>
                <div className="flex items-center box-border  justify-between border-b border-gray-500  my-4 pb-4 ">
                  {/* left */}
        ...
        {cartItems.length > 0 ? (
          <button
            ...
            } px-8 mt-8 rounded py-3 text-white bg-red-600 w-full`}
          >
            Remove all
          </button>
        ) : (
          <Link to="/store">
            <button
              className={` px-8 mt-8 rounded py-3 text-white bg-blue-600 w-full`}
            >
